...
async function pocetnoKrajnje(r_0_id,r_1_id){
    ...
    let p_vr0 = await fetch(`http://localhost:8080/schedules/searchById/${r_0_id}`)
    .then(odg=>odg.json())
    .catch(err=>console.log("Greska u trazenju rasporeda: " + err));
    p_vr0_=p_vr0.start
    
    let k_vr0 = await fetch(`http://localhost:8080/schedules/searchById/${r_0_id}`)
    .then(odg=>odg.json())
    .catch(err=>console.log("Greska u trazenju rasporeda: " + err))
    k_vr0_=k_vr0.end

    let p_vr1 = await fetch(`http://localhost:8080/schedules/searchById/${r_1_id}`)
    .then(odg=>odg.json())
    .catch(err=>console.log("Greska u trazenju rasporeda: " + err));
    p_vr1_=p_vr1.start

    let k_vr1 = await fetch(`http://localhost:8080/schedules/searchById/${r_1_id}`)
    .then(odg=>odg.json())
    .catch(err=>console.log("Greska u trazenju rasporeda: " + err))
    k_vr1_=k_vr1.end
...
async function dodajDogadaje(dogadaji,rasp_id){
    for (let i = 0; index < dogadaji.length; i++) {
        ...
        const dodaj_dogadaje = await fetch(`http://localhost:8080/events/${novi_dogad}`)
        .then(odg=>odg.json())
        .catch(err=>console.log("Greska u trazenju rasporeda: " + err))
    }
}
export default async function NakonKolizije(d0, d1, koliz,naziv,r_0_id,r_1_id){
    ...
    const rasp_= await fetch(`http://localhost:8080/schedules/`)
    .then(odg=>odg.json())
    ...
    const spojeni_raspored = await fetch(`http://localhost:8080/schedules/${novo}`)
    .then(odg=>odg.json())
    .catch(err=>console.log("Greska u trazenju rasporeda: " + err))
